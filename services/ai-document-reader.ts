...
{
  "modelo_cobranca": "um de: Avulso entrada+êxito / Avulso só êxito / Recorrente, ou null",
...
async function updateProcessFields(processId: string, fields: ExtractedProcessFields) {
  ...
  // vara_comarca mapeia para coluna localizacao
  if (sanitized.vara_comarca) {
    updateData.localizacao = sanitized.vara_comarca;
    delete updateData.vara_comarca;
  }
...
async function ensureProcessoFromClienteFile(
  ...
  const { error } = await supabase.from("processos").insert({
    ...
    localizacao: safeProc.vara_comarca ?? null,
