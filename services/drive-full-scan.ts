...
function parseProcessFolderName(name: string): { number: string; actionType: string } {
  const match = name.match(/^\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}/);
  if (match) {
    const number = match[0];
    const rest = name.slice(number.length).replace(/^_/, "");
    return { number, actionType: folderNameToDisplayName(rest) };
  }
  return { number: name, actionType: "" };
}
