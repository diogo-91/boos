...
import type { Client, ProcessStatus } from "@/lib/types";
import type { ProcessFormValues } from "@/components/OperationalDataProvider";
import { PROCESS_STATUS_FILTERS } from "@/lib/domain";
...
const PROCESS_STATUS_OPTIONS = PROCESS_STATUS_FILTERS.filter(
  (s) => s !== "Todos"
) as ProcessStatus[];
    ...
    >
      <div className="grid gap-4 px-4 py-4 sm:px-5 sm:py-5 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <FormField label="Status do processo">
          <SelectInput
            value={values.status}
            onChange={(e) => updateField("status", e.target.value as ProcessStatus)}
          >
        ...
        <FormField label="Modelo de cobrança">
          ...
          >
            <option value="">—</option>
            <option>Avulso entrada+êxito</option>
            <option>Avulso só êxito</option>
